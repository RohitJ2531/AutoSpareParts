...
if(empty($f_name) || empty($l_name) || empty($email) || empty($password) || empty($repassword) ||
	...
	if(!(strlen($mobile) == 10)){
     ...
     if(!preg_match($cnic_v,$cnic)){
		echo "
			<div class='alert alert-danger'>
				<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
				<b>Aadhar Number $cnic is not valid</b><small> Fellow its For Example : xxxx-xxxx-xxxx </small>
			</div>
		";
		exit();
	}
    ...
    if(!(strlen($cnic) == 14)){
		echo "
			<div class='alert alert-danger'>
				<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
				<b>Aadhar Number must be 12 digit and TWO - Add </b><i><strong> Fellow its For Example : xxxx-xxxx-xxxx </strong></i>
			</div>
		";
		exit();
	}
	...
	if($count_cnic > 0)
	{
		echo "
			<div class='alert alert-danger'>
				...
				<b>Aadhar Number is already Register ! <small> Please Send Mail To Admin  OR Contact Us</small></b>
			</div>
		";
		exit();
